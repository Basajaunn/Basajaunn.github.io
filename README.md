# Welcome! 

## My name is David! Nice to meet you! Here is some info about me:
- Recent graduate of WGU with a Bachelors in Cybersecurity
- Learning to create more intermediate/advanced Python projects
- Linux obsessed!
- Love to learn, hardworking, and a great attention to detail!
- I hike everyday I can with my dog B!

---

### Some of my skills
- Networking
- Linux
- Cybersecurity labs and CTFs
- Programming languages: Python
    - Learning Bash and Powershell
    - Just began learning C for a better understanding of the relationship between hardware and software!

---

## Python Projects
### Password Strength Tester
Currently in development, will be here soon!
### Hash Cracker (Coming soon)
**For educational use only!!!**
Will crack common hashes to show knowledge of cryptography and python proficiency.
### Password generator: [See it here](https://github.com/Basajaunn/working-demos/blob/main/Password-gen/passwordgen.py)

## C Projects
### Keylogger (Coming soon)
**For educational uses and showing my C knowledge only!**
This will track keystrokes of a user when the file is executed.

---

## Tryhackme and Certifications:
- My Tryhackme account: [My Profile](https://tryhackme.com/p/Basajaun)
- My Certifications: [Click me!](https://www.credly.com/users/david-whene/badges#credly)

---

### Tools/ other knowledge!
- Industry framework knowledge (e.g., NIST 800-53, PCI-DSS, MITRE ATT&CK) 
- Offensive tools (e.g., Burp suite, the harvester)
- Defensive tools (e.g., nmap, Nessus)
- Spanish proficiency: B1

---

## Contact me!
- *Email:* dwhenejr@gmail.com
- *Github:* [Basajaunn](https://github.com/Basajaunn)

---

### Other projects:
I host a soccer podcast breaking down games in La Liga and Serie A! Find it [here :)](https://linktr.ee/pitchsideplayback)

### Future Goals:
- Enhance all aspects of my Cybersecurity knowledge
- Work on more difficult python projects!
- Contribute to open source!
- **Become a polymath!**
